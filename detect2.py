    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                det = det[det[:, 0].argsort()]  # Sort by x1 (ascending)
                print(det)
                # Print results
        ...
        # Sort detections by Y-coordinate (y_max), then by X-coordinate (x_min)
        # detections_sorted = sorted(detections, key=lambda x: (-x[3], x[0]))  # Sort by y_max (descending), then x_min (ascending)
        # print(detections)
        # print(detections_sorted)
        # # Now process each detection in sorted order
        # reading = ""
        # for det in detections_sorted:
        #     x_min, y_min, x_max, y_max, conf, cls = det
        #     print(f"Class: {names[int(cls)]}, Confidence: {conf.item()}")
        #     reading += names[int(cls)]  # Append class name to reading
        #     print(f"{names[int(cls)]}")

        #     if save_txt:  # Write to file if needed
        #         xywh = (xyxy2xywh(torch.tensor([x_min, y_min, x_max, y_max]).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
        #         line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
        #         with open(f'{txt_path}.txt', 'a') as f:
        #             f.write(('%g ' * len(line)).rstrip() % line + '\n')

        #     # Save image or add bounding boxes, as necessary
        #     if save_img or save_crop or view_img:  # Add bbox to image
        #         c = int(cls)  # integer class
        #         label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
        #         annotator.box_label([x_min, y_min, x_max, y_max], label, color=colors(c, True))

        print(f"Final reading order: {reading}")
